...
let haxLiteral: number = 0xf00d; // 十六进制

// 3.字符串类型
//  string
//      一个字符系列，使用单引号或双引号表示字符串类型，反引号 ` 用于定义多行文本和内嵌表达式
// 例：
let name1: string = "asdfgh";
let years: number = 5;
...
//      用于标识方法返回值的类型，表明无返回值
// 例：
function hello(): void { alert("Hello!TS!"); }
hello();
...
// 遍历数组
var sites: string[] = new Array("Google", "YaHoo", "Apple", "Amerzen")
 ...
 */
//    例：
let tup1: [string, number, boolean] = ['asdf', 18, true];
let mytuple1: [number, string] = [10, 'asdf'];
mytuple1[0] = 120
mytuple1[1] = "asdf"
//    ⚠️注意：元素的类型和值是一一对应的
 ...
 */
function buildName1(firstName1: string, lastName1: string = "asd") {
    if (lastName1)
...
/**
 * 函数重载
 *  重载是方法名相同参数不同，返回类型可相同也可不同
 *  每个重载的方法或构造函数都必须有一个独一无二的参数类型列表
 */

// 参数类型不同
function disp3(string: any): void;
function disp3(number: any): void;

// 参数数量不同
function disp4(n1: number): void;
function disp4(x: number, y: number): void;

// 参数类型顺序不同
function disp5(n1: number, s1: string): void;
function disp5(s: string, n: number): void;
...
/**
 * 接口
 *  TS接口用来建立某种代码约定，使其他开发者调用某个方法或创建新的类时必须遵循接口所定义的代码约定
 */
/**
 *      接口定义：
...
/**
 * 接口作为数据结构
 *  这里没有使用implements语句实现接口，而是作为一个普通数据结构来使用
 */
// 定义一个类接口
interface Person{
    firstName: string;
...
class Employee{
    ...
    // 构造函数
    constructor(uname: string, age: string){
        this.uname = uname;
        this.age = age;
    }
}
var workers: Array<Person7> = []; // 指定数组只能放Person类型的元素
...
// 这里不用<>明确传入类型，编译器可以查看meString的值，然后将T设为对应的类型
